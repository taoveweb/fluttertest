...
class _FloatingDraggableWidgetState extends State<FloatingDraggableWidget>
          ...
          return Scaffold(
            ...
            body: GestureDetector(
              ...
              child: SizedBox(
                height: height,
                width: width,
                child: controller.isRemoved.value
                    ...
                    : Stack(
                        children: [
                          ...
                          AnimatedPositioned(
                            ...
                            curve:
                                top >= (height - widget.floatingWidgetHeight) ||
                                        left >=
                                            (width -
                                                widget.floatingWidgetWidth) ||
                            ...
                            child: GestureDetector(
                                ...
                                setState(() {
                                  if (isTabbed && isDragEnable) {
                                    isColliding = hasDeleteWidget &&
                                        hasCollision(
                                            containerKey1, containerKey2);
                                    top = _getDy(
                                        value.globalPosition.dy -
                                            (widget.floatingWidgetHeight),
                                        height);
                                    left =
                                        _getDx(value.globalPosition.dx, width);
                                    if (left < 41.w) {
                                      left = 41.w;
                                    }
                                  }
                              ...
                              onPanEnd: (value) {
                                setState(() {
                                  controller.dy.value = top;
                                  controller.dx.value = left;
  ...
  double _getDy(double dy, double totalHeight) {
    ...
    double currentTop;
    if (dy >= (totalHeight - widget.floatingWidgetHeight - 188.w)) {
      currentTop = (totalHeight - widget.floatingWidgetHeight - 188.w);
    } else {
      if (dy <= 161.w) {
        currentTop = widget.floatingWidgetHeight + 101.w;
      } else {
        currentTop = dy + widget.floatingWidgetHeight / 2;
      }
    }
    print('widget.floatingWidgetHeight ${widget.floatingWidgetHeight}');
    return currentTop;
  }
  ...
  double _getDx(double dx, double totalWidth) {
    ...
    double currentLeft;
    if (dx >= (totalWidth - widget.floatingWidgetWidth - 30.w)) {
      currentLeft = (totalWidth - widget.floatingWidgetWidth - 30.w);
    } else {
      if (dx <= 45.w) {
        currentLeft = 45.w;
      } else {
        currentLeft = dx - widget.floatingWidgetWidth / 2;
      }
    }
    if (dx <= 45.w) {
      currentLeft = 45.w;
    }
    print('widget.floatingWidgetWidth ${widget.floatingWidgetWidth}');
